...
width_local = int(round((local_map_x_right - local_map_x_left)/resolution_local))+1
...
for pair in source_M_m:
    """
    n = int(round((0 - local_map_x_left)/resolution_local))
    if pair[0]<= 0:

        ind_x = n - int(round((0-pair[0])/resolution_local))
        ind_y = int(round((local_map_y-pair[1])/resolution_local))-1
    else:
        ind_x = n + int(round((0+pair[0])/resolution_local))
        ind_y = int(round((local_map_y-pair[1])/resolution_local))-1
    """
    ind_x = int(round((pair[0]-local_map_x_left)/resolution_local))
    ind_y = int(round((local_map_y-pair[1])/resolution_local))-1
    index_cell.append([ind_x, ind_y])
    ind_x_cell.append(ind_x)
    ind_y_cell.append(ind_y)
print index_cell
plt.plot(ind_x_cell, ind_y_cell, label = "wtf")
plt.ylim([0,50])
plt.show()
