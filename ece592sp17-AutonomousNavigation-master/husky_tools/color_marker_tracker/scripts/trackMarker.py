...
if __name__== '__main__':
    ...
    pub=rospy.publisher('/color_marker_tracker/husky_tracker',tracking_info,queue=10)
    ros.init_node('color_tracker',anonymous=True)
    myNodeName=rospy.get_name()
...
# get marker Boundaries
    while True:
        if state== "getMarkerBoundaries":
            ...
            rosAllMarkersLocation=[]
            for marker in markers:
                rosAllMarkersLocation.append(markers[0].position_average_pixels)
            pub.publish(myNodeName,bot1.bot_position_current,bot1.bot_orientation_angle,rosAllMarkersLocation)
            updateMAP()
